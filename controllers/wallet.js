...
exports.manualPaymentRequest = async (req, res, next) => {
	try {
		...
		const newReq = await ManualPaymentRequest.create({
			user: userId,
			phone: phone,
			amount: amount,
...
exports.manualPaymentUpdateStatus = async (req, res, next) => {
	try {
		const { paymentReqId, status } = req.params;
		const paymentRequest = await ManualPaymentRequest.findById(
			paymentReqId
		);
		...
		if (status === 'approved') {
			...
			wallet.amount = wallet.amount + paymentRequest.amount;
			paymentRequest.status = 'approved';
			const newWallet = await wallet.save();
			const newReq = await paymentRequest.save();
			return res.json(newReq);
	...
	try {
		...
		form.parse(req, async (err, fields) => {
			...
			if (!address) {
				return res.status(400).json({
					error:
						'Your profile is missing address. Pease fill address first',
				});
			}
			if (!city) {
				return res.status(400).json({
					error:
						'Your profile is missing city. Pease fill city first',
				});
			}
			if (!postCode) {
				return res.status(400).json({
					error:
						'Your profile is missing postal code. Pease fill postal code first',
				});
			}
...
exports.success = async (req, res) => {
	try {
		...
		if (
			...
			response.element[0].status === 'VALIDATED'
		) {
			...
			if (paymentHistory.approved === false) {
				...
				const pair = await MessagePair.findOne({
					$or: [
						{
							$and: [
								{ user: savedWallet.user },
								{ receiver: savedWallet.user },
							],
						},
						{
				...
				if (!pair) {
					...
					return res.redirect(
						`${process.env.SITE_URL}/topup-order/success/${transactionId}`
					);
				}
			...
			PaymentHistory.findByIdAndDelete(transactionId)
				.then((del) => {
					return res.redirect(
						`${process.env.SITE_URL}/topup-order/fail/${transactionId}`
					);
				})
				.catch((err) => {
					console.log(err);
